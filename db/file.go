...
func OnFileUploadFinished(filehash string, filename string,
	...
	if err != nil {
		fmt.Println("Failed to prepare prepare statement., err", err.Error())
		return false
	}
	defer stmt.Close()
	...
	if rf, err := ret.RowsAffected(); nil == err {
		if rf <= 0 {
			fmt.Printf("file with hash:%s has been upload before", filename)
		}
		return true
	}
...
func UpdateFileLocation(filehash string, fileaddr string) bool {
	...
	if rf, err := ret.RowsAffected(); nil == err {
		if rf <= 0 {
			fmt.Printf("更新文件localtion失败, filehash:%s", filehash)
		}
		return true
	}
	return false
